# face_recognition_gpu.py
# GPU-accelerated Face Recognition (requires CUDA + onnxruntime-gpu)
...
if not CUDA_AVAILABLE:
    raise ImportError(
        "GPU Face Recognition requires CUDA support.\n"
...
class GPUFaceRecognition:
    """
    GPU-accelerated face recognition using OpenCV DNN with CUDA backend.
    Falls back to CPU DNN if GPU initialization fails.
    """
    ...
    def __init__(self, config: Dict = None):
        ...
        self.process_scale = self.config.get('process_scale', 0.75)
    ...
    def _initialize_models(self):
        """Initialize face detection with GPU support if available"""
        # Try to use OpenCV DNN with CUDA backend
        try:
            # Check if OpenCV was built with CUDA support
            if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                self.use_gpu = True
                logger.info(f"🎮 CUDA GPU detected: {cv2.cuda.getCudaEnabledDeviceCount()} device(s)")
            else:
                self.use_gpu = False
                logger.info("💻 No CUDA GPU detected, using CPU")
        except Exception:
            self.use_gpu = False
            logger.info("💻 OpenCV CUDA not available, using CPU")
        
        # Use Haar Cascade (works on both CPU and GPU)
        ...
        if self.face_detector.empty():
            ...
            mode = "GPU-accelerated" if self.use_gpu else "CPU"
            logger.info(f"✅ Face Detection initialized ({mode})")
    ...
    def _extract_face_feature(self, image: np.ndarray) -> Optional[np.ndarray]:
        """Extract face feature using histogram + texture analysis"""
        try:
            ...
            # GPU: could use cv2.cuda for resize if available
    ...
    def process_frame(self, frame: np.ndarray) -> Dict:
        """Process a frame for face recognition - returns dict with annotated frame"""
        self.frame_count += 1
        ...
        # GPU: could use cv2.cuda.resize if available
    ...
    def _find_match(self, feature: np.ndarray) -> Tuple[str, float]:
        """Find matching identity using batch comparison (GPU-optimized)"""
        best_match = "Unknown"
        best_score = 0.0
