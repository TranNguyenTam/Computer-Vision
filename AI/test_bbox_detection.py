"""
Test Bbox-based Fall Detection
Similar to test_fall_detection.py but uses BboxFallDetector
"""
...
import yaml
from src.bbox_fall_detector import BboxFallDetector
...
def test_video(video_path: str, save_output: bool = False, debug: bool = False):
    """Test bbox fall detection on video"""
    
    if debug:
        logging.getLogger().setLevel(logging.DEBUG)
        logging.getLogger('src.bbox_fall_detector').setLevel(logging.DEBUG)
    
    # Load config
    config = load_config()
    bbox_config = config.get('bbox_fall_detection', {})
    
    logger.info(f"🎥 Testing video: {video_path}")
    logger.info(f"📋 Config: {bbox_config}")
    
    # Initialize detector
    detector = BboxFallDetector(bbox_config)
    ...
    output_writer = None
    if save_output:
        output_path = Path(video_path).stem + '_bbox_output.mp4'
    ...
    while True:
        if not paused:
            ...
            if result['fall_detected']:
                ...
                # Save fall image
                fall_image_path = f'data/uploads/fall_{fall_count}_frame_{frame_count}.jpg'
            ...
            # Display
            cv2.imshow('Bbox Fall Detection Test', annotated)
        else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test Bbox Fall Detection')
