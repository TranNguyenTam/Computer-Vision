"""
Test Action Recognition Fall Detection
"""
...
import yaml
from src.action_fall_detector import ActionFallDetector
...
def test_video(video_path: str, save_output: bool = False, debug: bool = False):
    """Test action fall detection on video"""
    
    if debug:
        logging.getLogger().setLevel(logging.DEBUG)
        logging.getLogger('src.action_fall_detector').setLevel(logging.DEBUG)
    
    # Load config
    config = load_config()
    action_config = config.get('action_fall_detection', {})
    
    logger.info(f"🎥 Testing video: {video_path}")
    logger.info(f"📋 Config: {action_config}")
    
    # Initialize detector
    detector = ActionFallDetector(action_config)
    ...
    output_writer = None
    if save_output:
        output_path = Path(video_path).stem + '_action_output.mp4'
    ...
    while True:
        if not paused:
            ...
            if result['fall_detected']:
                ...
                logger.warning(f"🚨 FALL #{fall_count} detected at frame {frame_count}")
                
                # Save fall image
                fall_image_path = f'data/uploads/fall_action_{fall_count}_frame_{frame_count}.jpg'
                cv2.imwrite(fall_image_path, frame)
                logger.info(f"💾 Saved fall image: {fall_image_path}")
                
                # Log action scores
                if result['action_scores']:
                    logger.info(f"   Action scores: {result['action_scores']}")
            ...
            # Display
            cv2.imshow('Action Fall Detection Test', annotated)
        else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test Action Fall Detection')
