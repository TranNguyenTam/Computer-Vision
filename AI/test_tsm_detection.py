...
import yaml
from src.tsm_fall_detector import TSMFallDetector
...
def test_video(video_path: str, save_output: bool = False, debug: bool = False):
    """Test TSM fall detection on video"""
    
    if debug:
        logging.getLogger().setLevel(logging.DEBUG)
        logging.getLogger('src.tsm_fall_detector').setLevel(logging.DEBUG)
    
    # Load config
    config = load_config()
    tsm_config = config.get('tsm_fall_detection', {})
    
    logger.info(f"🎥 Testing video: {video_path}")
    logger.info(f"📋 Config: {tsm_config}")
    
    # Initialize detector
    detector = TSMFallDetector(tsm_config)
    ...
    output_writer = None
    if save_output:
        output_path = Path(video_path).stem + '_tsm_output.mp4'
    ...
    while True:
        if not paused:
            ...
            if result['fall_detected']:
                ...
                # Save fall image
                fall_image_path = f'data/uploads/fall_tsm_{fall_count}_frame_{frame_count}.jpg'
            ...
            # Display
            cv2.imshow('TSM Fall Detection Test', annotated)
        else:
            cv2.imshow('TSM Fall Detection Test', annotated)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test TSM Fall Detection')
